...
export async function POST(request: Request) {
  try {
    const { text, designStyle, contentType, useClaudeModel = true } = await request.json();
    ...
    console.log('收到生成请求，文本长度:', text.length, '使用Claude模型:', useClaudeModel);
    ...
    if (useClaudeModel) {
      ...
      // 使用DeepSeek模型（OpenRouter API）
      try {
        console.log('使用OpenRouter API调用DeepSeek模型...');
        const response = await fetchWithTimeout(
          'https://openrouter.ai/api/v1/chat/completions',
          {
            ...
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer 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`
            },
            body: JSON.stringify({
        ...
        if (!response.ok) {
          ...
          try {
            ...
            try {
              const textContent = await responseClone.text();
              console.error('OpenRouter API返回非JSON响应:', textContent);
              errorData = { error: `API响应格式错误: ${response.status} ${response.statusText}` };
            } catch (textError) {
              console.error('无法读取响应内容:', textError);
              errorData = { error: `API响应读取失败: ${response.status} ${response.statusText}` };
        ...
        const data = await response.json();
        console.log('OpenRouter API响应数据结构:', Object.keys(data));
        ...
        try {
          ...
          if (data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) {
            ...
            if (responseText.includes('<!DOCTYPE html>') || responseText.includes('<html')) {
              ...
              // 从开始标记到结束的内容
              let endIndex = responseText.lastIndexOf('</html>');
              if (endIndex !== -1) {
                endIndex += 7; // '</html>'.length
                const htmlContent = responseText.substring(startIndex, endIndex);
                console.log('提取的HTML内容长度:', htmlContent.length);
                return NextResponse.json({ html: htmlContent });
              }
            ...
            console.error('无法从API响应中提取HTML内容');
            console.error('响应内容:', responseText);
            return NextResponse.json(
              { error: 'API响应格式不符合预期，无法提取HTML内容' },
              { status: 500 }
            );
          }
        } catch (error) {
        ...
        // 处理超时错误
        if (error instanceof Error && error.name === 'AbortError') {
          console.error('OpenRouter API请求超时');
